                        ...
                        <div class="bg-white rounded-[3rem] md:rounded-[4rem] p-8 md:p-14 shadow-[0_40px_100px_-25px_rgba(0,0,0,0.06)] border border-slate-50 relative overflow-hidden animate-gemay">
                            ...
                            <div class="relative z-10 flex flex-col items-center text-center">
                                <div class="mb-8 relative">
                                    ...
                                    <div class="absolute -top-2 -right-2 md:-top-3 md:-right-3 w-10 h-10 bg-white border-4 border-slate-50 rounded-full flex items-center justify-center text-teal-500 shadow-xl">
                                ...
                                @if(!$latestDelivery->is_survey_filled)
                                    <button onclick="openSurveyModal({{ $latestDelivery->id }})" class="w-full sm:w-auto bg-slate-900 text-white font-extrabold px-10 md:px-12 py-5 rounded-[1.8rem] shadow-2xl hover:bg-teal-600 transition-all transform active:scale-95 text-xs tracking-[0.2em] flex items-center justify-center gap-4">
                                ...
                                @else
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 w-full">
                                        ...
                                        <a href="{{ asset('storage/'.$doc['path']) }}" target="_blank" class="flex flex-col items-center gap-4 p-7 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2.5rem] hover:bg-teal-50 hover:border-teal-400 hover:scale-[1.03] transition-all group shadow-sm">
                                            <div class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-teal-600 shadow-sm border border-slate-100 group-hover:bg-teal-600 group-hover:text-white transition-colors duration-300">
                                                <i class="fas {{ $doc['icon'] }} text-xl"></i>
                                            </div>
                                            <div class="space-y-1">
                                                <p class="text-[10px] font-extrabold text-slate-700 uppercase tracking-widest">{{ $doc['label'] }}</p>
                                                <span class="text-[9px] font-bold text-teal-500 uppercase tracking-tighter opacity-0 group-hover:opacity-100 transition-opacity">Klik Unduh PDF</span>
                                            </div>
                                        </a>
                                        @endforeach
                                    </div>
                                            ...
                                            <td class="py-5 text-right">
                                                @if($item->is_survey_filled)
                                                    ...
                                                    <div class="flex justify-end gap-2">
                                                        ...
                                                        <a href="{{ asset('storage/'.$item->file_kwintansi) }}" target="_blank" class="w-9 h-9 flex items-center justify-center bg-white border border-slate-200 text-slate-400 hover:text-teal-600 hover:border-teal-200 rounded-xl transition-all shadow-sm" title="Unduh Kwitansi">
                                                            <i class="fas fa-receipt text-xs"></i>
                                                        </a>
                                                    </div>
                                                @else
                                                    <button onclick="openSurveyModal({{ $item->id }})" class="text-[10px] font-extrabold text-teal-600 hover:underline underline-offset-4 tracking-widest">UNLOCK SEKARANG</button>
                                                @endif
                                            </td>
                                        </tr>
                                        @empty
