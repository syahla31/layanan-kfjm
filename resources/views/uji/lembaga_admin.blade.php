...
<body class="bg-slate-50 text-slate-800 antialiased">
    ...
    @php
        ...
        // Ambil semua user dengan role 'user' dan kategori 'uji'
        if (!isset($users)) {
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <!-- SIDEBAR DESKTOP (Warna Hijau Gelap) -->
        ...
        <div class="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50 relative w-full">
            ...
            <div class="hidden md:block">
                @include('components.uji-header', [
                    'title' => 'Data Lembaga',
                    'subtitle' => 'Manajemen data lembaga uji terdaftar'
                ])
            </div>
            ...
            <main class="flex-1 overflow-x-hidden overflow-y-auto p-4 md:p-6 lg:p-8 space-y-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p class="text-sm text-slate-500 mt-1">Total <span class="font-bold text-teal-700">{{ $totalLembaga }}</span> lembaga aktif</p>
                    </div>
                    
                    <div class="relative group w-full sm:w-64">
                        <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-slate-400 group-focus-within:text-teal-500 transition-colors">
                            <i class="fas fa-search"></i>
                        </div>
                        <input type="text" id="searchInput" onkeyup="filterTable()" class="block w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-100 transition-all shadow-sm" placeholder="Cari nama atau kode...">
                    </div>
                </div>
                                ...
                                <tr class="table-row-hover transition-colors group">
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="flex items-center gap-3">
                                            <div class="w-9 h-9 rounded-full bg-teal-50 text-teal-600 flex items-center justify-center text-xs font-bold border border-teal-100 shadow-sm shrink-0 uppercase">
                                                {{ substr($u->name, 0, 1) }}
                                            </div>
                                    ...
                                    <td class="px-6 py-5">
                                        <span class="bg-teal-50 text-teal-700 px-2 py-1 rounded-md text-xs font-mono font-bold border border-teal-100">
                                            {{ $u->kode_instansi ?? '-' }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-6 py-5 text-center">
                                        <button onclick="showDetail('{{ $u->name }}', '{{ $u->kode_instansi }}', '{{ $u->email }}', '{{ $u->created_at->format('d M Y') }}')" class="text-slate-400 hover:text-teal-600 p-2 rounded-lg hover:bg-teal-50 transition-all" title="Lihat Detail">
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
            <div class="flex min-h-full items-center justify-center p-4 text-center">
                <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all w-full max-w-sm border border-slate-100">
                    ...
                    <div class="bg-gradient-to-r from-teal-600 to-teal-800 px-6 py-5 flex justify-between items-center text-white">
                        <h3 class="text-lg font-bold flex items-center gap-2">
                            <i class="fas fa-id-card"></i> Detail Lembaga
                        </h3>
                        <button onclick="closeModal()" class="text-teal-100 hover:text-white bg-white/10 hover:bg-white/20 p-2 rounded-lg transition-colors">
                    ...
                    <div class="px-6 py-6 space-y-5">
                        ...
                        <div class="flex flex-col items-center sm:flex-row sm:items-center sm:gap-4 mb-2 text-center sm:text-left">
                            <div class="w-16 h-16 rounded-full bg-teal-50 flex items-center justify-center text-teal-600 text-2xl font-bold border-2 border-teal-100 uppercase mb-3 sm:mb-0" id="modalAvatar">
                                <!-- Initials -->
                            </div>
                            <div class="min-w-0">
                                <h4 class="text-lg font-bold text-slate-800 leading-tight break-words" id="modalName">Nama Lembaga</h4>
                                <span class="bg-teal-100 text-teal-700 text-xs px-2 py-0.5 rounded font-mono font-bold mt-1 inline-block" id="modalCode">KODE</span>
                            </div>
                        </div>
