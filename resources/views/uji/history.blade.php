...
<head>
    ...
    <title>Riwayat Disetujui | Admin Uji</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <style>
        body { font-family: 'Inter', sans-serif; }
        .table-row-hover:hover td { background-color: #f0fdfa; /* teal-50 */ }
        ...
        .modal-scroll::-webkit-scrollbar-thumb { background: #99f6e4; border-radius: 10px; }
        .modal-scroll::-webkit-scrollbar-thumb:hover { background: #5eead4; }
    </style>
</head>
<body class="bg-slate-50 text-slate-800 antialiased">
    ...
    @php
        ...
        $query = Submission::with(['user', 'files'])
                        ...
                        ->whereIn('type', [
                            'Radioterapi - Laporan Tahunan', 
                            'LUK - Laporan Tahunan',
                            'Eksterna - Laporan Tahunan',
                            'Nuklida - Laporan Tahunan',
                            'Radioaktivitas Lingkungan - Laporan Tahunan'
                        ]);
        ...
        $filterLabel = [
            'Semua' => 'Semua Dokumen',
            'Radioterapi - Laporan Tahunan' => 'Radioterapi',
            'LUK - Laporan Tahunan' => 'LUK',
            'Eksterna - Laporan Tahunan' => 'Eksterna',
            'Nuklida - Laporan Tahunan' => 'Nuklida',
            'Radioaktivitas Lingkungan - Laporan Tahunan' => 'Radioaktivitas Lingkungan'
        ];
        ...
        <div class="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50 relative w-full">
            ...
            <div class="md:hidden bg-white border-b border-slate-200 px-4 py-3 flex justify-between items-center z-20">
                <div class="flex items-center gap-2">
                    ...
                    <span class="font-bold text-slate-800 uppercase tracking-tight">SI-LAB UJI</span>
                </div>
            ...
            <div class="hidden md:block">
                @include('components.uji-header', [
                    'title' => 'Arsip Dokumen',
                    'subtitle' => 'Daftar laporan lembaga & dokumen persetujuan'
                ])
            </div>
            ...
            <main class="flex-1 overflow-x-hidden overflow-y-auto p-4 md:p-6 lg:p-8 space-y-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p class="text-sm text-slate-500 mt-1">Total <span class="font-bold text-teal-700">{{ $histories->count() }}</span> arsip tersimpan</p>
                    </div>
                    ...
                    <form id="filterForm" method="GET" action="{{ url()->current() }}" class="relative min-w-[260px] z-30">
                        ...
                        <!-- Trigger Button -->
                        <button type="button" onclick="toggleDropdown()" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 shadow-sm hover:border-teal-400 hover:shadow-md transition-all flex items-center justify-between group focus:outline-none focus:ring-2 focus:ring-teal-100">
                        ...
                        <div id="dropdownMenu" class="absolute right-0 top-full mt-2 w-full bg-white rounded-xl shadow-xl border border-slate-100 hidden transform origin-top scale-95 opacity-0 transition-all duration-200 overflow-hidden">
                            <div class="p-1.5 flex flex-col gap-1">
                                ...
                                <div onclick="selectFilter('{{ $val }}')" class="px-3 py-2.5 rounded-lg cursor-pointer flex items-center gap-3 text-sm font-medium transition-colors {{ $filterType == $val ? 'bg-teal-50 text-teal-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                                    <i class="fas fa-layer-group w-4 text-center {{ $filterType == $val ? 'text-teal-500' : 'text-slate-400' }}"></i>
                                    <span class="flex-1">{{ $label }}</span>
                                    @if($filterType == $val) <i class="fas fa-check text-teal-500 text-xs"></i> @endif
                                </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <table class="w-full text-sm text-left text-slate-600 min-w-[900px] md:min-w-0">
                            <thead class="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-200">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 font-bold tracking-wider text-center">Dokumen Lembaga</th>
                                    <th class="px-6 py-4 font-bold tracking-wider text-center">Status / SK Admin</th>
                                ...
                                <tr class="table-row-hover transition-colors group">
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="flex items-start gap-3">
                                            <div class="w-8 h-8 rounded-lg bg-teal-50 border border-teal-100 flex items-center justify-center text-teal-600 text-xs font-bold shrink-0 uppercase">
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="flex flex-col gap-1.5">
                                            ...
                                            @endphp
                                            <span class="{{ $badgeColor }} border text-[9px] px-2 py-0.5 rounded-full font-bold w-fit uppercase tracking-tighter">{{ $scope }}</span>
                                            <span class="text-slate-800 font-bold text-sm line-clamp-1" title="{{ $item->title }}">{{ $item->title }}</span>
                                        </div>
                                    </td>

                                    <!-- File Dokumen Lembaga -->
                                    <td class="px-6 py-5 text-center">
                                        <a href="{{ asset('storage/' . $item->file_path) }}" target="_blank" class="inline-flex items-center gap-2 text-xs font-bold text-teal-700 hover:text-teal-900 group/link transition-all bg-teal-50 px-3 py-1.5 rounded-lg border border-teal-100 hover:border-teal-300 shadow-sm" title="Buka Laporan Terakhir Lembaga">
                                            <i class="far fa-file-pdf text-rose-500"></i>
                                            <span class="group-hover/link:underline">Lihat Laporan</span>
                                        </a>
                                    </td>

                                    <!-- Status / Surat Balasan (SK) -->
                                    <td class="px-6 py-5 text-center">
                                        @php
                                            $finalAdminFile = $item->admin_file ?: ($item->files->whereNotNull('admin_file')->sortByDesc('version')->first()->admin_file ?? null);
                                        @endphp

                                        @if($finalAdminFile)
                                            <a href="{{ asset('storage/' . $finalAdminFile) }}" target="_blank" class="inline-flex items-center gap-1.5 text-emerald-600 hover:text-emerald-800 font-bold text-xs bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-100 transition-colors shadow-sm" title="Download Surat Keputusan / Balasan Admin">
                                                <i class="fas fa-certificate"></i> Unduh SK
                                            </a>
                                        @else
                                            <div class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 border border-emerald-100 text-xs font-bold shadow-sm">
                                                <i class="fas fa-check-circle"></i>
                                                <span>Baru Disetujui</span>
                                            </div>
                                        @endif
                                    </td>
                                    ...
                                    <!-- Jejak -->
                                    <td class="px-6 py-5 text-center">
                                        <button onclick='openHistoryModal(@json($item->files ?? []), "{{ $item->status }}", "{{ $item->title }}")' class="text-slate-400 hover:text-teal-600 p-2 rounded-full hover:bg-teal-50 transition-all" title="Lihat Perjalanan Dokumen">
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-3xl border border-slate-100">
                    ...
                    <!-- Header -->
                    <div class="bg-gradient-to-r from-teal-800 to-teal-950 px-6 py-5 flex justify-between items-center text-white shadow-md relative overflow-hidden">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-teal-200 mt-1" id="historyTitle">Detail perjalanan dokumen</p>
                        </div>
                        <button onclick="closeHistoryModal()" class="text-teal-200 hover:text-white bg-white/10 hover:bg-white/20 rounded-xl p-2 transition-all active:scale-95 z-10">
        ...
        function openHistoryModal(files, currentStatus, docTitle) {
            ...
            if(!files || files.length === 0) {
                ...
                cleanFiles.forEach((file, index) => {
                    ...
                    let colorClass = isLatest ? 'bg-teal-600 text-white ring-4 ring-teal-100' : 'bg-white border-2 border-slate-200 text-slate-500';
                    let actionTitle = isStart ? "Inisiasi / Awal" : "Unggahan Lembaga";

                    let userFileHTML = isStart ? '' : `
                        <div class="mt-2 flex items-center gap-3 bg-teal-50/50 p-2 rounded-lg border border-teal-100 cursor-pointer hover:bg-teal-50 transition-all" onclick="window.open('/storage/${file.file_path}', '_blank')">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-[9px] text-teal-600">Klik untuk melihat</p>
                            </div>
                        </div>`;

                    let adminFeedbackHTML = '';
                    if (isLatest) {
                        adminFeedbackHTML = `<div class="mt-3 pt-2 border-t border-slate-100">
                            <span class="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded text-[10px] font-bold border border-emerald-200">DISETUJUI (FINAL)</span>
                        </div>`;
                    } else if (file.admin_note || file.admin_file) {
                        adminFeedbackHTML = `
                            <div class="mt-3 pt-3 border-t border-slate-100">
                                <span class="bg-amber-100 text-amber-700 px-2 py-0.5 rounded text-[10px] font-bold border border-amber-200">REVISI</span>
                                ...
                                ${file.admin_file ? `<a href="/storage/${file.admin_file}" target="_blank" class="mt-2 inline-flex items-center gap-2 text-[10px] font-bold text-teal-700"><i class="fas fa-paperclip"></i> Lihat Lampiran Admin</a>` : ''}
                            </div>`;
                    }
                    ...
                    const itemHTML = `
                        <div class="relative flex gap-6 pb-8 last:pb-0">
                            ...
                            <div class="relative z-10 flex-shrink-0 w-8 h-8 rounded-full ${colorClass} flex items-center justify-center border-2 border-white text-[10px] font-bold">
                                ${versionLabel}
                            </div>
