...
<body class="bg-slate-50 text-slate-800 antialiased overflow-hidden text-left">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex-1 flex flex-col h-screen overflow-hidden relative">
            ...
            <div class="hidden lg:block">
                @include('components.pelatihan-header', ['title' => 'Evaluasi Surveilan', 'subtitle' => 'Penilaian Akreditasi Lembaga Pelatihan'])
            </div>
            ...
            <main class="flex-1 overflow-y-auto p-4 lg:p-10 space-y-8 no-scrollbar text-left">
                ...
                <div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6 animate-slide-up">
                    <div class="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm flex flex-col items-center md:flex-row md:items-center gap-4 lg:gap-5 text-center md:text-left">
                        <div class="w-14 h-14 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center text-xl shadow-inner shrink-0"><i class="fas fa-archive"></i></div>
                        <div>
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        <div class="bg-white rounded-[2rem] border border-slate-100 p-6 lg:p-8 hover:shadow-xl transition-all group relative overflow-hidden text-center md:text-left">
                            <div class="flex flex-col md:flex-row items-center justify-between gap-6 relative z-10 text-center md:text-left">
                                ...
                                <button type="button" data-submission='@json($item)' onclick="openEvaluateModal(this)" class="w-full md:w-auto bg-slate-900 text-white px-10 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-secondary transition-all shadow-xl active:scale-95">
    ...
    <div id="evaluateModal" class="fixed inset-0 z-[110] hidden items-center justify-center p-3 lg:p-4 text-left">
        ...
        <div class="relative bg-white rounded-[2rem] lg:rounded-[2.5rem] w-full max-w-6xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden animate-pop-in border border-white text-left">
            <div class="bg-primary p-6 md:p-8 text-white flex justify-between items-center shrink-0">
                <div class="text-left">
                    <h3 class="text-lg lg:text-xl font-black uppercase tracking-tight text-left">Instrumen Evaluasi Survailen</h3>
                    <p id="evalModalTitle" class="text-[10px] text-blue-200 font-bold uppercase tracking-widest mt-1.5 flex items-center gap-2 text-left"><i class="fas fa-university"></i> <span></span></p>
                </div>
            ...
            <form id="evaluateForm" method="POST" action="" enctype="multipart/form-data" class="flex-1 flex flex-col overflow-hidden bg-slate-50/30 text-left">
                ...
                <div class="flex-1 overflow-y-auto p-5 lg:p-10 space-y-8 no-scrollbar text-left">
                    <div class="bg-white p-5 rounded-2xl border border-slate-100 flex items-center justify-between gap-6 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 text-left">
                        ...
                        <div class="space-y-5 text-left">
                            <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6 text-left">
                                ...
                                <div class="pt-4 border-t border-slate-50 text-left">
                                ...
                                <p class="text-[9px] text-slate-400 font-bold italic mt-2 text-left">* Maksimal 2MB per file.</p>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="p-8 bg-white border-t border-slate-100 flex justify-end items-center gap-6 shrink-0">
                    <button type="button" onclick="closeEvaluateModal()" class="text-[11px] font-black uppercase text-slate-400 tracking-[0.2em] hover:text-rose-500 transition-colors">Batal</button>
                    <button type="submit" class="bg-slate-900 text-white px-12 py-5 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] hover:bg-secondary transition-all shadow-xl active:scale-95"><i class="fas fa-save mr-2"></i> Simpan Penilaian</button>
                </div>
            </form>
        </div>
    </div>
            ...
            categories.forEach((cat, idx) => {
                ...
                cat.files.forEach(fileObj => {
                    ...
                    if (path) {
                        ...
                        filesHtml += `<a href="${storageBase}/${cleanPath}" target="_blank" class="inline-flex items-center px-3 py-1.5 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white transition-all text-[10px] font-black"><i class="fas fa-file-pdf mr-2"></i>${fileObj.n}</a>`;
                    } else {
                            ...
                            <div class="flex justify-center gap-2">
                                ${[1, 2, 3, 4].map(s => `
                                    <div class="relative text-center">
                                        <input type="radio" name="scores[${cat.key}]" id="s_${idx}_${s}" value="${s}" class="hidden score-btn score-${s}" onchange="calc()" ${currentScore == s ? 'checked' : ''} required>
                                        <label for="s_${idx}_${s}" class="score-label w-10 h-10 flex items-center justify-center rounded-xl bg-white text-slate-300 text-sm font-black cursor-pointer border-2 border-slate-100 text-center">${s}</label>
                                    </div>
                                `).join('')}
                            </div>
                        </td>
