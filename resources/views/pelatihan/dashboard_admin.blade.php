...
<body class="bg-slate-50 text-slate-800 antialiased">
    ...
    @php
        ...
        $allowedTypes = ['KAK', 'Laporan Kinerja'];
        ...
        $countPending = Submission::where('category', 'pelatihan')
            ->whereIn('type', $allowedTypes)
            ->where('status', 'pending')
            ->count();
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <div id="mobileSidebar" class="fixed inset-0 z-50 hidden">
            ...
            <div
                class="absolute left-0 top-0 bottom-0 w-64 bg-blue-900 shadow-xl transform transition-transform duration-300">
                @include('components.pelatihan-sidebar')
            </div>
        </div>
        ...
        <div class="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50 relative w-full">
            ...
            <div
                ...
                <div
                    class="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 text-xs font-bold border border-blue-200">
            ...
            <div class="hidden md:block">
                @include('components.pelatihan-header', [
                    ...
                    'subtitle' => 'Dashboard Verifikasi KAK & Laporan Kinerja',
                ])
            </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
                    <div
                        ...
                        <div class="relative z-10">
                            <div class="flex items-center gap-3 mb-2">
                                <span
                                    class="bg-amber-100 text-amber-600 p-2 rounded-lg text-xs font-bold whitespace-nowrap">
                                    <i class="fas fa-inbox"></i> Inbox
                                </span>
                                <span class="text-xs font-bold text-slate-400 uppercase tracking-wide truncate">Perlu
                                    Tindakan</span>
                            </div>
                            ...
                            <div class="w-full bg-slate-100 h-1.5 rounded-full mt-4 overflow-hidden">
                                <div class="bg-amber-500 h-full rounded-full"
                                    style="width: {{ $countPending > 0 ? '100%' : '5%' }}"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <div class="relative z-10">
                            <div class="flex items-center gap-3 mb-2">
                                <span
                                    class="bg-emerald-100 text-emerald-600 p-2 rounded-lg text-xs font-bold whitespace-nowrap">
                                    <i class="fas fa-calendar-check"></i> Bulan Ini
                                </span>
                                <span
                                    class="text-xs font-bold text-slate-400 uppercase tracking-wide truncate">Disetujui</span>
                            </div>
                            <div class="flex items-end gap-2">
                    ...
                    <div
                        ...
                        <div class="relative z-10">
                            <div class="flex items-center gap-3 mb-2">
                                <span
                                    class="bg-rose-100 text-rose-600 p-2 rounded-lg text-xs font-bold whitespace-nowrap">
                                    <i class="fas fa-exclamation-circle"></i> Bulan Ini
                                </span>
                                <span
                                    class="text-xs font-bold text-slate-400 uppercase tracking-wide truncate">Dikembalikan</span>
                            </div>
                            <div class="flex items-end gap-2">
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
                        <div>
                            <h3 class="text-lg font-bold text-slate-800 flex items-center gap-2">
                                <i class="fas fa-list-ul text-blue-600"></i> Antrian Dokumen Masuk
                            </h3>
                            <p class="text-sm text-slate-500">Daftar dokumen KAK & Lapkin yang belum diproses</p>
                        </div>

                        @if ($submissions->count() > 0)
                            <div
                                class="bg-blue-600 text-white px-4 py-1.5 rounded-full text-xs font-bold shadow-md flex items-center gap-2 animate-pulse w-fit">
                                <span class="w-2 h-2 bg-white rounded-full"></span>
                                {{ $submissions->count() }} Dokumen Baru
                            </div>
                        @else
                            <div
                                class="bg-slate-100 text-slate-500 px-4 py-1.5 rounded-full text-xs font-bold flex items-center gap-2 w-fit">
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    @forelse($submissions as $index => $item)
                                        <tr class="table-row-hover transition-colors group">
                                            <td
                                                class="px-4 py-4 md:px-6 md:py-5 text-center text-slate-400 font-mono text-xs">
                                                {{ $index + 1 }}
                                            </td>
                                            <td class="px-4 py-4 md:px-6 md:py-5 whitespace-nowrap">
                                                <div class="flex flex-col">
                                                    ...
                                                    <div class="flex items-center gap-1.5 mt-1">
                                                        ...
                                                        <span
                                                            class="text-xs text-slate-500">{{ $item->created_at->format('H:i') }}
                                                            WIB</span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-4 py-4 md:px-6 md:py-5">
                                                <div class="flex items-start gap-3">
                                                    <div
                                                        class="hidden sm:flex w-9 h-9 rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 text-indigo-600 items-center justify-center text-xs font-bold border border-indigo-200 flex-shrink-0">
                                                        {{ substr($item->user->name ?? '?', 0, 1) }}
                                                    </div>
                                                    <div class="min-w-0">
                                                        <p
                                                            class="font-bold text-slate-800 text-sm line-clamp-1 break-all">
                                                            {{ $item->user->name ?? 'Unknown' }}</p>
                                                        <div class="flex items-center gap-2 mt-1">
                                                            <span
                                                                class="bg-slate-100 text-slate-500 text-[10px] px-2 py-0.5 rounded border border-slate-200 font-mono truncate">
                                                                {{ $item->user->kode_instansi ?? 'NO-CODE' }}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <td class="px-4 py-4 md:px-6 md:py-5">
                                                <div class="flex flex-col gap-2">
                                                    <div
                                                        ...
                                                        @if ($item->type == 'KAK')
                                                            <span
                                                                class="bg-indigo-50 text-indigo-700 border border-indigo-100 text-[10px] font-bold px-2 py-0.5 rounded w-fit">KAK</span>
                                                        @else
                                                            <span
                                                                class="bg-teal-50 text-teal-700 border border-teal-100 text-[10px] font-bold px-2 py-0.5 rounded w-fit">LAPKIN</span>
                                                        @endif
                                                        <p class="font-medium text-slate-800 text-sm line-clamp-1 break-all"
                                                            title="{{ $item->title }}">{{ $item->title }}</p>
                                                    </div>
                                                    <a href="{{ asset('storage/' . $item->file_path) }}"
                                                        target="_blank"
                                                        class="flex items-center gap-2 text-xs font-bold text-blue-600 hover:text-blue-800 w-fit group/link transition-colors bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100 hover:border-blue-300">
                                            ...
                                            <td class="px-4 py-4 md:px-6 md:py-5">
                                                <div
                                                    class="flex flex-row sm:flex-row items-center justify-center gap-2">
                                                    <button
                                                        ...
                                                        title="Setujui">
                                                        <i
                                                            class="fas fa-check text-sm w-4 h-4 md:w-5 md:h-5 flex items-center justify-center"></i>
                                                    </button>
                                                    <button
                                                        ...
                                                        title="Tolak / Revisi">
                                                        <i
                                                            class="fas fa-times text-sm w-4 h-4 md:w-5 md:h-5 flex items-center justify-center"></i>
                                                    </button>
                                                </div>
                                            </td>
                                        </tr>
                                        ...
                                        <tr>
                                            <td colspan="5" class="px-6 py-20 text-center">
                                                <div class="flex flex-col items-center justify-center">
                                                    <div
                                                        class="w-20 h-20 md:w-24 md:h-24 bg-slate-50 rounded-full flex items-center justify-center mb-4 border border-slate-100">
                                                        <i
                                                            class="fas fa-clipboard-check text-3xl md:text-4xl text-slate-300"></i>
                                                    </div>
                                                    <h3 class="text-slate-800 font-bold text-lg">Semua Selesai!</h3>
                                                    <p class="text-slate-500 text-sm mt-1">Tidak ada antrian dokumen
    ...
    <div id="verifyModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog"
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-slate-100 scale-95 opacity-0"
                    id="modalPanel">
                    <form id="verifyForm" method="POST" enctype="multipart/form-data" action="">
                        @csrf
                        <div
                            class="bg-gradient-to-r from-slate-50 to-white px-6 py-5 border-b border-slate-100 flex items-start gap-4">
                            <div id="modalIconBg"
                                class="flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-slate-100 shadow-sm border border-white">
                                <i id="modalIcon" class="fas fa-check text-slate-600 text-lg"></i>
                            </div>
                            <div class="flex-1 pt-1">
                                <h3 class="text-lg font-bold text-slate-900 leading-tight" id="modalTitle">Konfirmasi
                                </h3>
                                <p class="text-xs text-slate-500 mt-1 line-clamp-1 font-medium" id="modalDesc">
                                    Keterangan tindakan...</p>
                            </div>
                            <button type="button" onclick="closeModal()"
                                class="text-slate-400 hover:text-slate-600 transition-colors bg-white hover:bg-slate-100 p-2 rounded-lg">
                        ...
                        <div class="px-6 py-6 space-y-5">
                            <div class="space-y-2 bg-slate-50 p-4 rounded-xl border border-slate-200"
                                ...
                                <label
                                    ...
                                    <span
                                        class="text-[10px] bg-slate-200 px-1.5 py-0.5 rounded text-slate-500">Opsional</span>
                                </label>
                                <input type="file" name="admin_file"
                                    class="block w-full text-xs text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-white file:text-blue-600 hover:file:bg-blue-50 border border-slate-300 rounded-lg cursor-pointer bg-white"
                                    accept=".pdf">
                                <p class="text-[10px] text-slate-400 mt-1 flex items-center gap-1" id="fileInputHelp">
                                    <i class="fas fa-info-circle"></i> <span>Dokumen SK atau surat balasan
                                        resmi.</span>
                                </p>
                            </div>
                            <div class="space-y-2">
                        ...
                        <div class="bg-slate-50 px-6 py-4 flex flex-row-reverse gap-3 border-t border-slate-100">
                            <button type="submit" id="modalBtn"
                                class="inline-flex w-full justify-center rounded-xl bg-slate-800 px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-slate-200 hover:shadow-none hover:bg-slate-700 sm:w-auto transition-all active:scale-95">Konfirmasi</button>
                            <button type="button" onclick="closeModal()"
                                class="inline-flex w-full justify-center rounded-xl bg-white border border-slate-300 px-5 py-2.5 text-sm font-bold text-slate-700 shadow-sm hover:bg-slate-50 sm:w-auto transition-all">Batal</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
        ...
        function openModal(action, id, title, type) {
            ...
            if (action === 'approve') {
                ...
                iconBg.className =
                    "flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-emerald-100 text-emerald-600 border border-emerald-200";
                ...
                iconBg.className =
                    "flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-rose-100 text-rose-600 border border-rose-200";
