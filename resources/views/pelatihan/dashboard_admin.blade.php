...
<body class="bg-slate-50 text-slate-800 antialiased">
    ...
    @php
        ...
        // FILTER: Hanya Tampilkan KAK dan Laporan Kinerja
        $allowedTypes = ['KAK', 'Laporan Kinerja'];

        // Ambil Antrian (Pending) - Prioritas FIFO
                ...
                <div class="flex flex-col gap-4">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
                        ...
                        @if($submissions->count() > 0)
                            <div class="bg-blue-600 text-white px-4 py-1.5 rounded-full text-xs font-bold shadow-md flex items-center gap-2 animate-pulse w-fit">
                                    ...
                                    <tr class="table-row-hover transition-colors group">
                                        ...
                                        <td class="px-4 py-4 md:px-6 md:py-5">
                                            <div class="flex items-start gap-3">
                                                <div class="hidden sm:flex w-9 h-9 rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 text-indigo-600 items-center justify-center text-xs font-bold border border-indigo-200 flex-shrink-0">
                                        ...
                                        <td class="px-4 py-4 md:px-6 md:py-5">
                                            <div class="flex flex-col gap-2">
                                                <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                                                    @if($item->type == 'KAK')
                                                        <span class="bg-indigo-50 text-indigo-700 border border-indigo-100 text-[10px] font-bold px-2 py-0.5 rounded w-fit">KAK</span>
                                                    @else
                                                        <span class="bg-teal-50 text-teal-700 border border-teal-100 text-[10px] font-bold px-2 py-0.5 rounded w-fit">LAPKIN</span>
                                                    @endif
                                        ...
                                        <td class="px-4 py-4 md:px-6 md:py-5">
                                            <div class="flex flex-row sm:flex-row items-center justify-center gap-2">
                                                <!-- Tombol Approve (Safe using data attributes) -->
                                                <button 
    ...
    <!-- MODAL POPUP VERIFIKASI (DESIGN MODERN) -->
                    ...
                    <form id="verifyForm" method="POST" enctype="multipart/form-data" action="">
                        ...
                        <div class="px-6 py-6 space-y-5">
                            ...
                            <div class="space-y-2 bg-slate-50 p-4 rounded-xl border border-slate-200" id="fileInputContainer">
                                ...
                                <input type="file" name="admin_file" class="block w-full text-xs text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-white file:text-blue-600 hover:file:bg-blue-50 border border-slate-300 rounded-lg cursor-pointer bg-white" accept=".pdf">
                            ...
                            <div class="space-y-2">
                                ...
                                <textarea name="admin_note" rows="3" class="block w-full rounded-xl border-slate-300 bg-white p-3 text-sm focus:border-blue-500 focus:ring-blue-500 shadow-sm resize-none" placeholder="Tuliskan catatan atau alasan revisi di sini..."></textarea>
                            </div>
                        </div>
    ...
    <script>
        ...
        function openModal(action, id, title, type) {
            ...
            if(action === 'approve') {
                ...
                // Logic Lapkin (Opsional) vs KAK (Wajib)
                if (type === 'Laporan Kinerja') {
                    fileLabel.innerHTML = `<span>Upload Surat Balasan</span> <span class="text-[10px] bg-slate-200 px-1.5 py-0.5 rounded text-slate-500">Opsional</span>`;
                    fileHelp.innerText = "Tidak wajib upload file untuk Laporan Kinerja.";
                } else {
                    fileLabel.innerHTML = `<span>Upload SK / Surat (PDF)</span> <span class="text-[10px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded">Wajib</span>`;
                    fileHelp.innerHTML = '<i class="fas fa-exclamation-circle text-red-500"></i> Wajib upload bukti verifikasi.';
                }
